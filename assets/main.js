...
const no = parseInt(file.split("-")[0])

// halaman
let halaman = localStorage.getItem("halaman")
...
if (no) {
    halaman[no] = [file.split("-")[1].split(".")[0], document.title]
    localStorage.setItem("halaman", JSON.stringify(halaman))
}
...
const jawab = async () => {
    ...
    const url = location.origin + paths.join("/") + "/" + (no + 1) + "-" + md5(jawaban).substring(0, 10) + ".html"
...
document.getElementById("jawaban")?.addEventListener("keydown", (e) => {
    if (e.key == "Enter") {
        jawab()
    }
})
document.getElementById("btn-jawaban")?.addEventListener("click", jawab)

// lain-lain
const abc = () => {
    let abc = ''
    for (let i = 97; i <= 122; i++) {
        abc += '<span style="color:' + abcColor(i) + '">' + String.fromCharCode(i) + '</span>'
    }
...
const abcColor = (i) => {
    ...
    if (i == 100 || i == 101 || i == 109 || i == 111 || i == 115) {
        return colors[0]
    }
...
const buka = (elem) => {
    if (Math.random() < 0.5) {
        elem.src = "assets/dead.png"
    } else {
        elem.src = "assets/cat.png"
    }
...
const toc = () => {
    ...
    for (let i = 0; i < sorted.length; i++) {
        const item = halaman[sorted[i]]
        const url = location.origin + paths.join("/") + "/" + sorted[i] + "-" + item[0] + ".html"

        const node = document.createElement("li")
        const childNode = document.createElement("a")
        const textNode = document.createTextNode(item[1])

        childNode.setAttribute("href", url)
